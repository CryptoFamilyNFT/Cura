...
export const SignUp = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // TODOAPI call
        await signUpMutation({ email, password }).unwrap()
            .then((response) => {
                console.log("Sign up successful:", response);
                dispatch(signUp(response)); // Aggiorna Redux con l'utente registrato
                setIsSuccess(true);
            })
            .catch((error) => {
    ...
    return (
        <div className="flex justify-center items-center absolute w-full h-full">
            ...
            {user.isAuthenticated && (
                <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-t from-[#23687D] to-[#A1C877] opacity-50 z-0">
                    <h4 className="text-emerald-700 text-2xl font-bold">Already Conneted!</h4>
                </div>
            )}
