...
export default function Navbar({ handleTheme }) {
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-full h-12 bg-gradient-to-b from-emerald-80 to-transparent bg-opacity-70 backdrop-blur-lg z-10 flex flex-row`}
    >
      {/** left box side */}
      <div className="flex items-center p-2 flex-row flex-1">
        ...
        <div className="border-l border-emerald-500 h-full mx-2">
          {menuButton.map((button) => (
            <button onClick={(e) => navigate(e.target.name)} name={button.link}
              className={`ml-2 hover:bg-emerald-900 hover:text-emerald-50 border-emerald-500 border-1 text-emerald-600 font-semibold py-1 px-3 rounded`}
              key={button.title}
            >
              {button.title}
            </button>
