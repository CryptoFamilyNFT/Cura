...
const QuizPage = () => {
  ...
  const getLabelClasses = (value) => {
    const baseClasses =
      "block text-[#F4F7E1] border border-green-100 py-2 px-4 rounded-lg cursor-pointer shadow-md";
    const selectedClasses =
      "bg-gradient-to-t from-[#23687D] to-purple-700 text-purple-200";
    const hoverClasses = "hover:bg-purple-200/30 hover:text-purple-200";
    const defaultClasses = "bg-green-100/15";

    return `${baseClasses} ${
      selected === value ? selectedClasses : defaultClasses
    } ${hoverClasses}`;
  };
      ...
      <div className="bg-green-100/15 p-6 rounded-xl shadow-md">
        ...
        {dataRes.quizs.length > 0 ? (
          dataRes.quizs.map((quiz) => (
            <div key={quiz.id}>
              <p className="text-3xl text-[#23687D] text-center my-4">
                Ti senti generalmente felice e soddisfatto/a della tua vita?
              </p>
